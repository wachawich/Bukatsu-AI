...
def get_activity_ai(model):
    ...
    try:
        ...
        # le_act = prediction_data['le_act']
        # X_test = prediction_data['X_test']
        
        print("prediction_data", prediction_data)

        # # แทนที่จะใช้ activity_id ให้ใช้ activity_type_name_th แทน
        # # แปลง activity_type_enc เป็นชื่อ activity_type_name_th
        # activity_names_th = le_act.inverse_transform(X_test['activity_type_enc'])

        # # สร้าง list ของ tuple (activity_type_name_th, score)
        # ranked = sorted(
        #     zip(activity_names_th, scores),
        #     key=lambda x: -x[1]
        # )

        # rank_map = {
        #     act_name_th: {"rank": i, "score": round(score, 4)}
        #     for i, (act_name_th, score) in enumerate(ranked, 1)
        # }

        # # ใส่ rank & score ลง enriched_data โดยใช้ activity_type_name_th ใน enriched_data เทียบกับ rank_map
        # for item in enriched_data:
        #     act_name_th = item.get('activity_type_name_th')
        #     item['rank'] = rank_map.get(act_name_th, {}).get('rank')
        #     item['score'] = rank_map.get(act_name_th, {}).get('score')
        ...
        # เรียงตาม rank
        #enriched_data.sort(key=lambda x: x.get('rank', float('inf')))
